...
const lessons = [
  lesson('semantic-scopes', 'scopes-pointed', 'Pointed truth', 'Select a world at which the formula is true.',
    ...
    { instruction: 'Select an evaluation world where ◇p is true.', formula: '◇p', scope: 'pointed', targetTruth: true, evaluationWorld: 'w1', worlds: [w('w0', '', 40), w('w1', '', 40, 220), w('w2', 'p', 360, 60), w('w3', '', 360, 220)], edges: [edge('w0', 'w2'), edge('w1', 'w3')], editable: ['evaluation'], workspacePresentation: { evaluation: true } },
    hints('Compare the successors of w0 and w1.', 'Only w0 reaches a p-world.', 'Set w0 as the evaluation world.'), 'At w0, the accessible world w2 witnesses ◇p.'),
  ...
  lesson('semantic-scopes', 'scopes-comparison', 'Three scopes, one model', 'Compare pointed, model-global, and frame-valid truth for one formula.',
    ...
    { instruction: 'Choose the correct truth profile, then verify all three scopes.', formula: '◇p', scope: 'pointed', targetTruth: true, evaluationWorld: 'w0', scopeComparison: { evaluationWorld: 'w0' }, prediction: { kind: 'statement-choice', prompt: 'Which truth profile is correct for ◇p?', expectedChoice: 'pointed-true-model-false-frame-false', mustBeCorrect: true, statementChoices: [
