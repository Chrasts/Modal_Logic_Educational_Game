...
describe('sandbox user interface', () => {
  ...
  it('shows one Intro CTA and switches compact Campaigns tabs', async () => {
    const user = userEvent.setup()
    render(<App />)
    expect(screen.getAllByRole('button', { name: /START INTRO TO MODAL LOGIC|CONTINUE INTRO TO MODAL LOGIC/ })).toHaveLength(1)
    expect(screen.queryByText('Internal lesson engine')).not.toBeInTheDocument()
    await user.click(screen.getByRole('button', { name: 'Campaigns' }))
    expect(screen.getByRole('tab', { name: 'Intro to Modal Logic' })).toHaveAttribute('aria-selected', 'true')
    expect(screen.queryByText('Countermodel Hunter')).not.toBeInTheDocument()
    await user.click(screen.getByRole('tab', { name: 'General Challenges' }))
