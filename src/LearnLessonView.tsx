...
export function LearnLessonView({ lesson, stage, predictionAnswer, predictionMessage, exampleStep, onStage, onPrediction, onExampleStep, onBeginTask, onBack }: {
  ...
  useEffect(() => { titleRef.current?.focus() }, [lesson.id])
  ...
  return <section className="content-screen learn-lesson-screen" aria-labelledby="learn-lesson-title">
    <header className="lesson-header"><button type="button" className="text-button" onClick={onBack}>← Back to Learn</button><p className="eyebrow">Learn Modal Logic · lesson</p><h1 id="learn-lesson-title" ref={titleRef} tabIndex={-1}>{lesson.title}</h1><p>{lesson.learningObjective}</p></header>
    ...
    {stage === 'prediction' && prediction && <article className="lesson-card prediction-card"><p className="eyebrow">Predict before testing</p><h2>{prediction.prompt}</h2>{prediction.kind === 'truth' ? <div className="choice-row"><button className={predictionAnswer === 'true' ? 'selected' : ''} type="button" onClick={() => onPrediction('true')}>True</button><button className={predictionAnswer === 'false' ? 'selected' : ''} type="button" onClick={() => onPrediction('false')}>False</button></div> : prediction.kind === 'world-choice' ? <div className="choice-row">{prediction.worldChoices?.map((choice) => <button className={predictionAnswer === choice ? 'selected' : ''} type="button" key={choice} onClick={() => onPrediction(choice)}>{choice}</button>)}</div> : prediction.kind === 'statement-choice' ? <div className="statement-choice-grid" role="radiogroup" aria-label="Statement answer">{prediction.statementChoices?.map((choice) => <button role="radio" aria-checked={predictionAnswer === choice.id} className={predictionAnswer === choice.id ? 'selected' : ''} type="button" key={choice.id} onClick={() => onPrediction(choice.id)}>{choice.label}</button>)}</div> : prediction.kind === 'scope-truth' ? <p>Make the three scope predictions in the workspace before verification.</p> : <div className="choice-row">{prediction.modelChoices?.map((choice) => <button className={predictionAnswer === choice.id ? 'selected' : ''} type="button" key={choice.id} onClick={() => onPrediction(choice.id)}>Model {choice.id}</button>)}</div>} {predictionMessage && <p className="prediction-message">{predictionMessage}</p>}<button className="primary-action" type="button" disabled={!predictionAnswer && prediction.kind !== 'scope-truth'} onClick={() => onStage('task')}>Continue to task</button></article>}
    ...
    {stage === 'completion' && <article className="lesson-card feedback-card"><p className="eyebrow">Task complete</p><h2>{lesson.title}</h2><p>{lesson.successExplanation}</p><button className="primary-action" type="button" onClick={onBack}>Back to Learn</button></article>}
  </section>
}
