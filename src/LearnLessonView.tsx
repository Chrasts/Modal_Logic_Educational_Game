...
export function LearnLessonView({ lesson, stage, predictionAnswer, predictionMessage, exampleStep, onStage, onPrediction, onExampleStep, onBeginTask, onBack }: {
  ...
  return <section className="content-screen learn-lesson-screen" aria-labelledby="learn-lesson-title">
    ...
    {stage === 'completion' && <article className="lesson-card feedback-card"><p className="eyebrow">Lesson complete</p><h2>{lesson.title}</h2><p>{lesson.successExplanation}</p><button className="primary-action" type="button" onClick={onBack}>Back to Learn</button></article>}
  </section>
}
