...
export function LearnLessonView({ lesson, stage, predictionAnswer, predictionMessage, exampleStep, onStage, onPrediction, onExampleStep, onBeginTask, onBack }: {
  ...
  return <section className="content-screen learn-lesson-screen" aria-labelledby="learn-lesson-title">
    <header className="lesson-header"><button type="button" className="text-button" onClick={onBack}>← Intro to Modal Logic</button><p className="eyebrow">Introductory campaign · lesson</p><h1 id="learn-lesson-title">{lesson.title}</h1><p>{lesson.learningObjective}</p></header>
    ...
    {stage === 'completion' && <article className="lesson-card feedback-card"><p className="eyebrow">Lesson complete</p><h2>{lesson.title}</h2><p>{lesson.successExplanation}</p><button className="primary-action" type="button" onClick={onBack}>Return to Intro to Modal Logic</button></article>}
  </section>
}
