...
interface GuideViewProps {
  ...
  readonly onOpenModelSandbox: () => void
}

export function GuideView({ tab, hasCurrentMission, onTabChange, onReturnToMission, onReplayWelcome, onReplayControls, onReplayTour, onOpenLearn, onOpenModelSandbox }: GuideViewProps) {
  ...
  return <section className="content-screen guide-screen" aria-labelledby="guide-screen-title">
    ...
    <div className="guide-actions"><button type="button" className="secondary-button" onClick={onReplayWelcome}>Replay Welcome to Modal Logic</button><button type="button" className="secondary-button" onClick={onReplayControls}>Replay Learn the Controls</button><button type="button" className="secondary-button" onClick={onReplayTour}>Replay workspace tour</button><button type="button" className="secondary-button" onClick={onOpenLearn}>Open Learn</button><button type="button" className="secondary-button" onClick={onOpenModelSandbox}>Try in Model Sandbox</button></div>
    ...
    <div className="guide-page-grid">
      ...
      {tab === 'controls' && <><MapControlsReference onReplayTour={onReplayTour} /><article><h2>Local data</h2><p>Data exports or imports model JSON and resets the saved Model Sandbox or learning progress independently.</p></article></>}
