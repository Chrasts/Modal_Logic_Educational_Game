...
export function PredictionInput({ prediction, answer, onAnswer, scope = 'pointed', availableWorldIds = [], propertyChoices = [], worldSelectionMode = 'control' }: PredictionInputProps) {
  ...
  if (prediction.kind === 'world-choice' || prediction.kind === 'counterexample-world') {
    ...
    return <select aria-label={prediction.kind === 'world-choice' ? 'Witness world answer' : 'Predicted counterexample world'} value={answer} onChange={(event) => onAnswer(event.target.value)}><option value="">Select a world</option>{worldChoices.map((id) => <option key={id}>{id}</option>)}</select>
  }
  if (prediction.kind === 'truth') return <div className="question-choice-row prediction-choice" role="radiogroup" aria-label="Truth answer">{truthLabels.map(([value, label]) => <button type="button" role="radio" aria-checked={answer === value} className={answer === value ? 'active' : ''} key={value} onClick={() => onAnswer(value)}>{label}</button>)}</div>
  ...
  if (prediction.kind === 'frame-property') return <select aria-label="Relational property answer" value={answer} onChange={(event) => onAnswer(event.target.value)}><option value="">Select a property</option>{relationChoices.map((property) => <option key={property}>{property}</option>)}</select>
