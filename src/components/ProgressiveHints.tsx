export function ProgressiveHints({ hints, revealed, onReveal }: { readonly hints: readonly string[]; readonly revealed: number; readonly onReveal: (index: number) => void }) {
  ...
  return <div className="progressive-hints">
    ...
    {revealed < hints.length && <button type="button" className="secondary-button" data-umami-event="hint-reveal" data-umami-event-hint-number={String(revealed + 1)} onClick={() => onReveal(revealed + 1)}>Reveal hint {revealed + 1}</button>}
  </div>
}
