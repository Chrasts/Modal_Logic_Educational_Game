...
export function buildQuestionFeedback({ attemptCount, detail, correct, lesson }: { readonly attemptCount: number; readonly detail: string; readonly correct: boolean; readonly lesson?: LearnLesson }): string {
  ...
  if (attemptCount === 2) return [detail, lesson?.commonMistake ?? lesson?.diagnosticFeedback?.objective].filter(Boolean).join(' ')
  return [detail, lesson?.commonMistake, lesson?.successExplanation].filter(Boolean).join(' ')
}
